...
const fastModel = process.env.GEMINI_FAST_MODEL || 'gemini-3.8-flash';
const deepModel = process.env.GEMINI_DEEP_MODEL || 'gemini-3.8-flash';
...
function historyToContents(history: Message[] = []) {
  return history.slice(-14).map(m => ({ role: m.role === 'assistant' ? 'model' : 'user', parts: [{ text: m.content }] }));
}

async function chat(req: VercelRequest, res: VercelResponse) {
  const input = req.body || {};
  const message = String(input.message || 'Analyze the attached content.').trim();
  ...
  try {
    const response = await gemini.models.generateContent({
      model: input.thinkingMode === 'DEEP' ? deepModel : fastModel,
      contents: [...historyToContents(input.history), { role: 'user', parts }],
      config: { systemInstruction: 'You are romsuperior ai, an advanced general-purpose assistant. Be accurate, helpful, concise when appropriate, and clearly state uncertainty. Never reveal private chain-of-thought.' }
    });
    json(res, 200, { reply: response.text || 'No response was returned.' });
  } catch (e) {
    console.error(e);
    json(res, 503, { message: 'AI service temporarily unavailable.' });
  }
}

async function research(req: VercelRequest, res: VercelResponse) {
  const url = String(req.body?.url || '').trim();
  if (!/^https?:\/\//i.test(url)) return json(res, 400, { message: 'Enter a valid http or https URL.' });
  try {
    ...
    console.error(e);
    json(res, 502, { message: 'Could not research that web page.' });
  }
}

async function agent(req: VercelRequest, res: VercelResponse) {
  const task = String(req.body?.message || '').trim();
  if (!task) return json(res, 400, { message: 'Agent task is required.' });
  ...
  try {
    ...
    console.error(e);
    json(res, 503, { message: 'Agent task temporarily unavailable.' });
  }
}

async function imageGen(req: VercelRequest, res: VercelResponse) {
  const prompt = String(req.body?.prompt || '').trim();
  if (!prompt) return json(res, 400, { message: 'Prompt is required.' });
  try {
    ...
    if (!image?.data) return json(res, 502, { message: 'No image was returned.' });
    json(res, 200, { image: image.data, mimeType: image.mimeType || 'image/png' });
  } catch (e) {
    console.error(e);
    json(res, 503, { message: 'Image generation temporarily unavailable.' });
  }
}

async function chats(req: VercelRequest, res: VercelResponse, userCtx: any) {
  if (!userCtx) return json(res, 401, { message: 'Sign in required.' });
  const { client, user } = userCtx;
  if (req.method === 'GET') {
    ...
    return json(res, 200, { chats: (data || []).map((x: any) => ({ id: x.client_id, title: x.title, messages: x.messages, updatedAt: x.updated_at })) });
  }
  if (req.method === 'POST') {
    const { id, title, messages } = req.body || {};
    if (!id || !title || !Array.isArray(messages)) return json(res, 400, { message: 'Chat id, title and messages are required.' });
  ...
  if (req.method === 'DELETE') {
    const id = String(req.query.path || '').split('/').pop();
    const { error } = await client.from('chats').delete().eq('owner_user_id', user.id).eq('client_id', id);
    if (error) return json(res, 500, { message: error.message });
